...
jQuery(function($) {
	fetch('https://api.giphy.com/v1/gifs/random?tag=thanks&rating=pg&api_key=dc6zaTOxFJmzC&limit=1').then(function(response) { 
		return response.json();
	}).then(function(my_result) {
		$('#fetch-result').html('<img src="'+my_result.data.fixed_height_downsampled_url+'" class="img-responsive">');
	});
});
